...
def run(model: str, camera_id: int, width: int, height: int) -> None:
  ...
  while time.time() - timer_start_time < 8:
...
@bot.message_handler(commands=['whereareyou'])
def sign_handler(message):
    if main():
        bot.send_message(message.chat.id, "I'm in the living room")
    else:
        bot.send_message(message.chat.id, "not in the living room")
